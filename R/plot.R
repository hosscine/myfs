...
#' @export
#'
setMargin <- function(top.4 = 4, left.4 = 4, bottom.5 = 5, right.2 = 2) graphics::par(mar = c(bottom.5, 
  left.4, top.4, right.2))

#' Changes margins to zeros
#'
#' @export
#'
setMarginZero <- function() graphics::par(mar = c(0, 0, 0, 0))
...
image2 <- function(X, col.rainbow = F, col.gray0 = F) {
  if (is.null(rownames(X))) 
    rn <- 1:nrow(X) else rn <- rownames(X)
  if (is.null(colnames(X))) 
    cn <- 1:ncol(X) else cn <- colnames(X)
  
  if (col.rainbow) 
    clr <- unique(X) %>% length %>% matlab.like else clr <- unique(X) %>% length %>% blue2red
  if (col.gray0) 
    clr[1] <- grDevices::gray.colors(20)[2]
  
  fields::image.plot(1:ncol(X), 1:nrow(X), t(apply(t(X), 1, rev)), axes = F, ann = F, col = clr)
...
databar <- function(X, grid = T) {
  ...
  graphics::plot(X, xlim = c(0.5, ncol(X) + 0.5), ylim = c(nrow(X) + 0.5, 0.5), type = "n", axes = F, 
    ann = F)
  ...
  for (x in 1:ncol(X)) {
    for (y in 1:nrow(X)) {
      if (X.normalize[y, x] != 0) 
        plotrix::gradient.rect(xleft = x - 0.5 + bar.width.margin, ybottom = y - 0.5 + bar.height.margin, 
          xright = x - 0.5 + (1 - bar.width.margin) * X.normalize[y, x]/100, ytop = y + 
          0.5 - bar.height.margin, col = grDevices::cm.colors(100)[1:X.normalize[y, x]])
